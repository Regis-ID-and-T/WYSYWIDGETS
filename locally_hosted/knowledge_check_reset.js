...
function clickedStuff(evnt){
	...
	if(keynum && (keynum == 0 || keynum == 1 || keynum == 13 || keynum == 32)){
		...
		while(!clickedThis.className || (clickedThis.className != "check_immediately" && clickedThis.className != "check_all" && clickedThis.className != "check_shortAnswer" && clickedThis.className != "check_showAnswer" && clickedThis.className != "check_resetAll")){clickedThis = clickedThis.parentNode;}
		...
		if(interactiveType == "immediately" || interactiveType == "all" || interactiveType == "shortAnswer" || interactiveType == "showAnswer" || interactiveType == "resetAll"){
...
var doStuff = {
	...
	resetAll: function(nothing){
		var theseResponseDivs = document.getElementsByTagName("DIV");
		for(var jmax = theseResponseDivs.length-1; jmax>=0; jmax--){
			if(theseResponseDivs[jmax].className && (theseResponseDivs[jmax].className == "feedbackText0" || theseResponseDivs[jmax].className == "feedbackText1")){
				theseResponseDivs[jmax].className = "feedbackText";
			}
		}
		var theseInputs = document.getElementsByTagName("INPUT");
		for(var imax=theseInputs.length-1; imax>=0; imax--){
			if(theseInputs[imax].checked){
				theseInputs[imax].checked=false;
			}
		}
	}
};
...
//			questionText = rowElements[1].textContent || rowElements[1].innerText;
	...
	resetButton.style.marginLeft = "2em"; resetButton.style.marginTop = "2em";
	resetButton.innerHTML = "Reset answers";
	resetButton.className = "check_resetAll";
	kcWrapper.appendChild(resetButton);
}
